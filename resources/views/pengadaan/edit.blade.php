...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('pengadaan.update', $pengadaan) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nomor_pengadaan" id="nomor_pengadaan" value="{{ $pengadaan->nomor_pengadaan }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
        </div>
        <div class="mb-4">
            <label for="tanggal" class="block text-sm font-medium text-gray-700">Tanggal</label>
            <input type="date" name="tanggal" id="tanggal" value="{{ $pengadaan->tanggal }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
        </div>
        <div class="mb-4">
            ...
            <select name="vendor_id" id="vendor_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @foreach ($vendor as $v)
                    <option value="{{ $v->id }}" {{ $v->id == $pengadaan->vendor_id ? 'selected' : '' }}>{{ $v->nama }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
            ...
            <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                <option value="pending" {{ $pengadaan->status == 'pending' ? 'selected' : '' }}>Pending</option>
                ...
                <option value="batal" {{ $pengadaan->status == 'batal' ? 'selected' : '' }}>Batal</option>
            </select>
        </div>
        <div class="mb-4">
            ...
            <div id="barang-list">
                @foreach ($pengadaan->details as $detail)
                    <div class="barang-item mb-4">
                        ...
                        <input type="number" name="jumlah[]" value="{{ $detail->jumlah }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        <label for="harga_satuan[]" class="block text-sm font-medium text-gray-700 mt-2">Harga Satuan</label>
                        <input type="number" name="harga_satuan[]" step="0.01" value="{{ $detail->harga_satuan }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>
                @endforeach
            </div>
